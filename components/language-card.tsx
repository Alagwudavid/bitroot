...
export function LanguageCard({
  ...
  return (
    ...
    >
      <CardContent className="p-5 px-6 relative"
            onClick={(e) => {
              e.stopPropagation(); // Prevent card's onClick from firing
              onLanguageSelect(language.name);
            }}>
        {isContinous ? <CircleCheck className="size-5 absolute top-2 right-2 text-green-500" /> : ""}
        ...
        <div className="w-full flex items-center flex-col mt-4">
            <div className="w-full flex items-center justify-center space-x-2 mb-1 text-gray-500 dark:text-[#fafafa]/60">
              <div className="flex items-center space-x-1 text-gray-500 dark:text-[#fafafa]/60">
                ...
                <span className="text-sm">{language.lessons} units</span>
              </div>
            ...
            <div className="w-full dark:text-[#8ddeed] text-sky-500 text-base text-center font-semibold">{language.learned} Learners</div>
        </div>
      </CardContent>
    </Card>
  );
